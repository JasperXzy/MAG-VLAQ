...
def parse_arguments():
    ...
    parser.add_argument("--unfreeze_dino_mode", type=str, default="frozen", choices=["frozen", "last2", "full", "lora", "multiconv"])
    ...
    parser.add_argument("--dino_mc_blocks", type=str, default="22_23", help="Block indices for MultiConv injection: 'all' or underscore-separated, e.g. '22_23'")
    parser.add_argument("--dino_mc_ratio", type=float, default=0.5, help="Bottleneck ratio for MultiConv adapter")
    parser.add_argument("--dino_mc_kernels", type=str, default="1_3_5", help="Conv kernel sizes for MultiConv adapter, underscore-separated odd ints")
    parser.add_argument("--lrdino_mc", type=float, default=None, help="Learning rate for DINOv2 MultiConv adapter params; falls back to args.lrpc when None")
    ...
    parser.add_argument("--unfreeze_utonia_mode", type=str, default="last1", choices=["frozen", "last1", "full", "lora", "mlp_adapter"], help="Utonia freeze mode: frozen=all frozen, last1=stage4 only (~40M/29%%), full=all (~137M), lora=inject LoRA into every Block, mlp_adapter=inject bottleneck MLP parallel to each Block's MLP")
    ...
    parser.add_argument("--utonia_mlp_stages", type=str, default="4", help="Encoder stages for MLP adapter injection: 'all' or underscore-separated, e.g. '4' or '2_3_4'")
    parser.add_argument("--utonia_mlp_ratio", type=float, default=0.25, help="Bottleneck ratio for Utonia MLP adapter")
    parser.add_argument("--utonia_mlp_dropout", type=float, default=0.0, help="Dropout on Utonia MLP adapter hidden activations")
    parser.add_argument("--lrutonia_mlp", type=float, default=None, help="Learning rate for Utonia MLP adapter params; falls back to args.lrpc when None")
